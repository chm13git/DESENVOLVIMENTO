...
$usuario='smm';
$senha='dfebdb6a';
$HTTP_PROXY='$usuario:$senha@proxy-armacao.mb:6060';
$HTTPS_PROXY='$usuario:$senha@proxy-armacao.mb:6060';
$NO_HTTP_PROXY='127.0.0.1,10.5.176.0/20,10.5.192.0/20,*.mb,*.mar.mil.br';
$curl='curl -x https://$HTTPS_PROXY -U $usuario:$senha';
...
#  $URL='http://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_sflux.pl?file=gfs.t12z.sfluxgrbf00.grib2&var_UGRD=on&var_VGRD=on&leftlon=0&rightlon=360&toplat=90&bottomlat=-90&dir=%2Fgfs.2018092012'
...
### Alterei os dados para os novos em 22/07/2019 - 1T Rambo ####### 
...
#$grb='gfs.t${HH}z.sfluxgrbf${FHR}.grib2';
$grb='gfs.t${HH}z.pgrb2.0p25.f${FHR}';
#$inv='gfs.t${HH}z.sfluxgrbf${FHR}.grib2.idx';
$inv='gfs.t${HH}z.pgrb2.0p25.f${FHR}.idx';
#
#
...
print "$url$grb -- $range";
      ...
      $err = system("$curl -f -v -s -r \"$range\" $url$grb -o ${OUTDIR}/${output}.tmp");
      $err = $err >> 8;
      if ($err != 0) {
      ...
      rename "${OUTDIR}/${output}.tmp", "${OUTDIR}/${output}";
