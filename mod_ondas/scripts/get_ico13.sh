...
if [ $# -lt 1 ]
   ...
   echo "               13km, horário                  "
   echo "                                              "
   echo "          ./get_ico13.sh hh yyyymmdd          "
   echo "                                              "
   echo "       ex: ./get_ico13.sh 00 20190716         "
   echo "+--------------------------------------------+"
   exit
fi
...
for VAR in $VARS; do
   ...
   VV='U_10M'
    ...
    ${p_cdo} mergetime ${DIRICONdados13}/icon_global_icosahedral_single-level_${AMD}${HSIM}_*_${VV}_reg.nc ${file_nc} 
...
${p_cdo} sellonlatbox,0,359.75,-90,90 ${WORKDIRICON13}/wnd_out.nc ${WORKDIRICON13}/wnd_sellon.nc
file_nc=ico13.${AMD}${HSIM}.nc 
${p_ncks} -4 -L 1 ${WORKDIRICON13}/wnd_sellon.nc ${WORKDIRICON13}/wnd_cp.nc
